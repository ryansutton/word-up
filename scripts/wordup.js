...
function addNewWordSubmission(word) {
    ...
    console.log('wordsumbmissions',model.wordSubmissions)
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            var theAnswer = response.results;
                //model.wordSubmissions.isRealWord = true;
            for (var i = 0; i < model.wordSubmissions.length; i++) {
                if (model.wordSubmissions[i].word == word) {
                    model.wordSubmissions[i].isRealWord = (theAnswer.length > 0);
                    break;
                }
            }
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        ...
        if (wordSubmission.isRealWord == true) {
            ...
            scoreChip.attr("class", "tag tag-default tag-primary tag-sm")
        } else {
           scoreChip.text("X");
           scoreChip.attr("class", "tag tag-default tag-sm tag-danger")
        }
        // TODO 18
...
function isDisallowedLetter(letter) {
    ...
    // letters = model.allowedLetters.indexOf(letter)
    //if (letters >= 0) {
    //    return false;
    //} else {
    //    return true;
...
function wordScore(word) {
    ...
    var wordScores = letters.map(function(submission) {
        return letterScore(submission);
    })
